 ...
 * 		1)searches for client by firstName and lastName (user inputted)
...
public class ReservationManagementPanelTwo extends MyPanel implements ActionListener, Reloadable, ItemListener
{
	//user input fields
	...
	//used throughtout program to help modify information
	int trainID;
	int newCapacity;
	
	public ReservationManagementPanelTwo() throws IllegalArgumentException
	{
		//initiating fields
		...
		findClientArea.add(new JLabel("First Name"));
		findClientArea.add(firstNameInput);
		findClientArea.add(new JLabel("Last Name"));
		findClientArea.add(lastNameInput);
		...
		reservationNumberInput.setEditable(false); // only for display
	...
	public void actionPerformed(ActionEvent e) throws IllegalArgumentException
	{
		...
		{	//if the user has inputed into the letterInput field, will display list of possible clients
			...
			{
				if (firstNameInput.getText().length() >=1 && lastNameInput.getText().length()>=1)
				{
				...
				{
					JOptionPane.showMessageDialog(this,"Please enter a first and last name");
				}
			}
